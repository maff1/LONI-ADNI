...
phenoLabels = gsub("\\.VISITm[0-9]{1,2}", "", colnames(filMetBetas))
...
ha_pheno = HeatmapAnnotation(
  pheno = phenoLabels,
  time = timeLabels,
  col = list(
    pheno = c("BPDIA" = "purple", "BPSYS" = "orange", "EXF" = "dark green",
              "LAN" = "light blue", "MEM" = "red", "MMSE" = "light green"),
    time = c("m12" = "yellow", "m24" = "pink", "m36" = "brown")
    ),
  gp = gpar(col = "black")
)
...
# MET ANNOTATIONS ------------------------------------------------------------ #
ranno <- read.csv("./DATA/NG-annotations.csv")[, c(1,4)]
colnames(ranno) <- c("name", "group")
ranno$name <- toupper(ranno$name)
metSelAnn <- ranno[match(rownames(filMetBetas), ranno$name),]

# ABBREVIATIONS
metSelAnn$groupAbbr <- abbreviate(metSelAnn$group, minlength = 4, named = FALSE)
rownames(filMetBetas) <- paste(rownames(filMetBetas), metSelAnn$groupAbbr, sep = "-")
# ---------------------------------------------------------------------------- #


pdf("./RESULTS/heatmap_lxm_time_interaction_selected_MET_LABELS_v2c.pdf")
Heatmap(filMetBetas, 
